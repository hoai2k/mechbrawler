...
import { previewCharacter, previewStage, claimCharacter, loadProgress, onLoadProgress } from "./assets.js";
...
function buildStageGrid() {
  ...
  for (const stage of STAGES) {
    ...
    // (src/stages.js, thumbFile). NOT `loading="lazy"`: these are ~29 kB each
    // now, they are warmed before the screen opens (menu_art.js), and lazy was
    // half of what made the grid fill in card by card in front of the player.
    ...
    img.addEventListener("error", () => {
      ...
      img.src = backgroundFile(stage);
    });
    img.src = thumbFile(stage);
    const label = document.createElement("span");
    label.append(stage.name);
    if (stage.desc) {
      const small = document.createElement("small");
      small.textContent = stage.desc;
      label.append(small);
    }
...
// about to look at after that". Menu pictures are HTML, so they are fetched when
// their element renders and not a moment sooner — which is the moment the screen
// opens, in front of the player. Asking for them a screen early turns every one
// of those into a cache hit (src/menu_art.js).
//
// The lists are cheap to build and the warmer skips anything already asked for,
// so this runs on every phase change rather than trying to be clever about which
// transitions matter.

/** Every picture the roster screen draws. The tile and the hero card are the
 *  same file here (rosterTileSrc is heroCardSrc), so this is one per mech rather
 *  than the two upstream needs. */
function selectScreenArt() {
  return CHARACTER_KEYS.map(rosterTileSrc);
}

/** Every picture the arena screen draws: one thumbnail per board. */
function stageScreenArt() {
  return STAGES.map((s) => thumbFile(s));
}

function warmForPhase(phase) {
  if (phase === "title") {
    // Next is the roster, then the arenas. And the Random board's full backdrop
    // is already worth queueing: the pump has spare cycles this early, and it is
    // the one stage that CANNOT be predicted from anything the player does later.
    warmMenuArt([...selectScreenArt(), ...stageScreenArt()]);
    previewStage(nextRandomStage());
  } else if (phase === "menu") {
    // On the roster now, so the arenas are next; the roster's own art is already
    // asked for and the warmer will not repeat it.
...
// backdrop goes to the front of the queue. The delay is what makes it a dwell
// rather than a sweep: steering across the grid with a pad brushes every card
...
//
// The draw used to happen when the wheel span, which meant the winning board's
// backdrop started downloading at the one moment the player is already waiting:
// the roulette lands, the match loads, and the stage nobody could have predicted
// has to arrive from cold. Sampling the RNG early changes nothing a player can
// observe — the pick is uniform, and it is still unknown to them until the wheel
// stops — but it means `previewStage` can have the backdrop in hand before the
// wheel is even spun.
...
/** Spend the held draw, so the NEXT Random is a fresh one. Without this a second
 *  Random lands on the same board as the first, for the rest of the session —
 *  which is the one thing sampling the RNG early must not change. */
function takeRandomStage() {
  const key = nextRandomStage();
  pendingRandomStage = null;
  return key;
}
...
export function setPhase(phase) {
  ...
  state.phase = phase;
  // The art of the screen AFTER this one, asked for while the player is reading
  // this one (warmForPhase). Only on a real change: re-entering the same phase
  // would throw away a queue that is already in flight for it.
  if (changed) warmForPhase(phase);
