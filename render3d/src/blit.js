...
/** Centre of mass as a fraction of body height, for a caller that did not say.
 *  Every real one comes in through `opts.comFrac` (backend.drawCharFrame, off
 *  body_points.comFrac), measured per mech and per state; this is the roster-wide
 *  constant that used to be the ONLY answer here. */
const COM_FRAC = 0.55;
...
export function blitPose(ctx, entry, charKey, x, y, opts = {}) {
  ...
  // The mass, in blitted pixels above the foot line. The old constant was a flat
  // 0.55 of the target height for every mech in every pose — and, because it
  // multiplied `targetPx` alone, it also quietly ignored `renderScale`, so a mech
  // drawn at any dial other than 1 turned about a point that was not on its body.
  // Both are gone: `rowsForBody * s` is the body's real drawn height in pixels,
  // and the fraction is this mech's own in this state.
  const comY = -(opts.comFrac ?? COM_FRAC) * rowsForBody * s;
