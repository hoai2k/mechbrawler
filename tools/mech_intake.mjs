...
//
// ONE EXCEPTION: each entry's `toon` block is ART DIRECTION, not a derivation
// from the export — the cel palette a fighter is painted in and the shade tint
// its own drawing shades with (tools/derive_cel_from_canonical.mjs writes them;
// the toon workbench edits them by hand). Nothing here can reconstruct that, so
// re-running this tool CARRIES IT ACROSS from whatever manifest is already on
// disk rather than dropping it. A mech that has none simply gets none.
//
...
// Carry the art direction across (see the exception above). Read late and
// defensively: a missing or unreadable manifest is a first run, not an error.
let previous = {};
try {
  previous = JSON.parse(readFileSync(OUT, "utf8")).characters || {};
} catch {}
let kept = 0;
for (const [id, entry] of Object.entries(characters)) {
  const toon = previous[id]?.toon;
  if (toon) { entry.toon = toon; kept++; }
}

const manifest = {
  comment: "GENERATED by tools/mech_intake.mjs from the mechs/ export — do not hand-edit, EXCEPT characters.<id>.toon, which is art direction (tools/derive_cel_from_canonical.mjs, or the toon workbench) and is carried across a regeneration. clips.<state>.glb names the mech's OWN exported animation driving that state; clips.<state>.freeze holds it at that time in seconds (resolveClip in render3d/src/loader.js).",
  characters,
};
writeFileSync(OUT, JSON.stringify(manifest, null, 2) + "\n");
console.log(`wrote ${OUT}: ${ids.length} mechs, ${kept} toon block(s) kept`);
