# Image requests — the archive of DELIVERED rounds

**This is the history document. Nothing on this page is outstanding** — the
live request list is [docs/image-requests.md](image-requests.md), and fulfilled
rounds move here so the open surface stays a one-screen read. The briefs are
kept verbatim rather than deleted, because a later redraw of one plate has to
agree with the brief the others were drawn to.

All 61 plates of the mech-conversion round — 33 power effects, 9
status/shared, 16 arena hazards, 3 UI — arrived in `assets/intake/`, were
landed by `tools/effects_intake.py`, and are in `assets/sprites/effects/` and
`assets/ui/`. The 14 garnish cards of the JJK-era round 18F
(`assets/sprites/garnish/`) are also all delivered, and as of the arena polish
pass all 14 are wired again (src/camera3d/garnish.js).
...
### Delivered, loaded, not yet hung — SINCE RESOLVED

(Kept as written at delivery time; all thirteen have since been hung — every
`stagefx:` plate now draws from `src/stage_fx.js` via `drawFx`.)
