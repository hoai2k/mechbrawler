...
**`/3d`**. The game, the sim, the sprites and the billboard path are untouched.
...
"3d": {
  ...
  drawCharFrame, currentFrame, cyclePhase,   // from /3d/src/backend.js
...
`init()` dynamic-imports the engine and the manifest, so nothing under `/3d`
(three.js included) loads unless `?render=3d` was typed. Three.js is already
vendored at `billboards/vendor/`; rather than a second copy, **hoist it to
`/vendor/three/`** shared by both backends (one version, one VENDOR.md, both
import sites updated) — the only cross-subsystem file move this plan makes.
...
3d/
  src/
    ...
    loader.js      .glb fetch/parse, clip retarget, all-or-nothing registration
    pose.js        animKey + animTime -> posed skeleton; on-twos quantisation;
                   the live layers: aim pitch, look-at, flinch, turnaround
    toon.js        the anime materials: ramp, shade-bias, rim, eye/face rules
    outline.js     inverted-hull ink pass, per-material width, crease lines
    scene.js       offscreen WebGL scene: character camera, stage light rig,
                   contact shadow, supersampled render target
    blit.js        texture -> 2D context, same placement maths as
                   billboards/src/blit.js (extracted shared where identical)
  assets/          approved runtime rigs + textures + manifest.json
  intake/          deliveries land here first; validate -> review -> apply
  workbench/       /3d/workbench/ — look-dev + clip review (§9)
  docs/
    plan.md        this file
...
Every knob defaults from one place (`3d/src/toon.js` constants) and is
...
**Workbench** (`/3d/workbench/`): the billboard workbench's shape — state
